...
#ifndef AEIF_COND_ALPHA_CLOPATH_H
#define AEIF_COND_ALPHA_CLOPATH_H
 ...
 */
extern "C" int aeif_cond_alpha_clopath_dynamics( double, const double*, double*, void* );
 ...
 * @param void* Pointer to model neuron instance.
 */
extern "C" int aeif_cond_alpha_clopath_dynamics_DT0( double, const double*, double*, void* );

/* BeginUserDocs: neuron, integrate-and-fire, adaptive threshold, conductance-based, Clopath pasticity

Short description
+++++++++++++++++

Conductance based exponential integrate-and-fire neuron model. It is an extension of
the aeif_cond_alpha model and capable of connecting to a Clopath synapse.

Description
+++++++++++

aeif_cond_alpha_clopath is the adaptive exponential integrate and fire neuron according
...
.. [2] Voltage-based STDP synapse (Clopath et al. 2010) on ModelDB
...
aeif_cond_alpha, iaf_cond_alpha, aeif_cond_exp, clopath_synpase

EndUserDocs */

class aeif_cond_alpha_clopath : public ClopathArchivingNode
{

public:
  aeif_cond_alpha_clopath();
  aeif_cond_alpha_clopath( const aeif_cond_alpha_clopath& );
  ~aeif_cond_alpha_clopath();
...
private:
  ...
  friend int aeif_cond_alpha_clopath_dynamics( double, const double*, double*, void* );

  // The next two classes need to be friends to access the State_ class/member
  friend class RecordablesMap< aeif_cond_alpha_clopath >;
  friend class UniversalDataLogger< aeif_cond_alpha_clopath >;
...
public:
  ...
  struct Buffers_
  {
    Buffers_( aeif_cond_alpha_clopath& );                  //!<Sets buffer pointers to 0
    Buffers_( const Buffers_&, aeif_cond_alpha_clopath& ); //!<Sets buffer pointers to 0

    //! Logger for all analog data
    UniversalDataLogger< aeif_cond_alpha_clopath > logger_;
  ...
  static RecordablesMap< aeif_cond_alpha_clopath > recordablesMap_;
};

inline port
aeif_cond_alpha_clopath::send_test_event( Node& target, rport receptor_type, synindex, bool )
{
  SpikeEvent e;
  e.set_sender( *this );
...
inline port
aeif_cond_alpha_clopath::handles_test_event( SpikeEvent&, rport receptor_type )
{
  if ( receptor_type != 0 )
  {
...
inline port
aeif_cond_alpha_clopath::handles_test_event( CurrentEvent&, rport receptor_type )
{
  if ( receptor_type != 0 )
  {
    throw UnknownReceptorType( receptor_type, get_name() );
...
inline port
aeif_cond_alpha_clopath::handles_test_event( DataLoggingRequest& dlr, rport receptor_type )
{
  if ( receptor_type != 0 )
  {
    throw UnknownReceptorType( receptor_type, get_name() );
...
inline void
aeif_cond_alpha_clopath::get_status( DictionaryDatum& d ) const
{
  P_.get( d );
  S_.get( d );
...
inline void
aeif_cond_alpha_clopath::set_status( const DictionaryDatum& d )
{
...
#endif // HAVE_GSL
#endif // AEIF_COND_ALPHA_CLOPATH_H
